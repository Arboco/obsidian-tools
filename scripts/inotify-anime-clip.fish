...
set screenshot_folder "/home/anon/ortup/important/notes/ortvault/resources/anime_db/media/clips/$folder_title"
...
inotifywait -m -e create --format '%w%f' $a_path | while read FILE
  ...
  if test $codec = 'hevc' 
    ...
    ffmpeg -i $FILE -c:v libx264 -crf 18 -preset slow -c:a aac -b:a 320k $screenshot_folder/$folder_title$timestamp.$file_end
