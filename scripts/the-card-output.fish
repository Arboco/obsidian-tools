...
while true
    for i in (cat /tmp/the-card_final_sorted_array)
        ...
        while string match -q false $permit_obtained
          ...
          /^$/ && flag {flag=0}
            ...
            set bat_code bash
            set codeblock_on false
            if cat $card_content | rg -q "```"
                set codeblock_on true
                if cat $card_content | rg -q "```[a-zA-Z]"
                    set bat_code (cat $card_content | rg -o '^```(\w+)' | sed 's/`//g')
                end
            end

            if string match -q true $codeblock_on
                awk '
                  BEGIN { in_block = 0 }
                  /^```/ {
                      in_block = !in_block
    ...
    if string match -q true $practice_mode
    else
        break
    end
end
